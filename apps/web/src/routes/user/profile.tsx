...
import { useAuthAccounts, useSession } from '@/hooks/use-session'
import { allowedProviders, getAccountInfo } from '@/lib/auth-providers'
import { orpc } from '@/lib/orpc'

export const Route = createFileRoute('/user/profile')({
  loader: ({ context }) => {
    void context.queryClient.prefetchQuery(orpc.user.authAccounts.queryOptions())
  },
  component: () => <Profile />,
})
...
export function Profile() {
  ...
  const { authAccounts, refetchAuthAccounts } = useAuthAccounts()
    ...
    <>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            {authAccounts.filter((a) => a.providerId !== 'credential').length > 0 ? (
              authAccounts
                  ...
                  return (
                    ...
                    >
                      ...
                      <Button
                        variant="destructive"
                        size="sm"
                        disabled={isDisconnecting === account.accountId || authAccounts.length <= 1}
                        onClick={() =>
          ...
          <div>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {allowedProviders
                  .filter(
                    (provider) => !authAccounts.some((acc) => acc.providerId === provider.provider),
                  )
