const translation = {
  common: {
    ...
    ImageUploadLegacyTip:
      '現在，您可以在起始表單中創建檔案類型變數。我們將來不再支持圖片上傳功能。',
  ...
  env: {
    envPanelTitle: '環境變數',
    envDescription:
      '環境變數可用於存儲私人信息和憑證。它們是唯讀的，並且可以在導出時與DSL文件分開。',
    envPanelButton: '添加變數',
    modal: {
      title: '添加環境變數',
      editTitle: '編輯環境變數',
  ...
  chatVariable: {
    panelTitle: '對話變數',
    panelDescription:
      '對話變數用於儲存 LLM 需要記住的互動資訊，包括對話歷史、上傳的檔案、使用者偏好等。這些變數可讀寫。',
  ...
  changeHistory: {
    ...
    hint: '提示',
    hintText:
      '您的編輯操作將被跟踪並存儲在您的設備上，直到您離開編輯器。此歷史記錄將在您離開編輯器時被清除。',
  ...
  blocksAbout: {
    ...
    'parameter-extractor':
      '利用 LLM 從自然語言內推理提取出結構化參數，用於後置的工具調用或 HTTP 請求。',
  ...
  nodes: {
    common: {
      ...
      errorHandle: {
        ...
        failBranch: {
          ...
          inLog:
            'Node 異常，將自動執行fail分支。節點輸出將返回錯誤類型和錯誤消息，並將其傳遞給下游。',
          customizeTip:
            '啟動fail分支後，節點引發的異常不會終止進程。相反，它將自動執行預定義的fail分支，允許您靈活地提供錯誤消息、報告、修復或跳過操作。',
        },
        partialSucceeded: {
    ...
    parameterExtractor: {
      ...
      reasoningMode: '推理模式',
      reasoningModeTip:
        '你可以根據模型對於 Function calling 或 Prompt 的指令響應能力選擇合適的推理模式',
    ...
    iteration: {
      ...
      parallelModeEnableDesc:
        '在並行模式下，反覆運算中的任務支援並行執行。您可以在右側的 properties 面板中進行配置。',
      answerNodeWarningDesc:
        '並行模式警告：反覆運算中的應答節點、對話變數賦值和持久讀/寫操作可能會導致異常。',
