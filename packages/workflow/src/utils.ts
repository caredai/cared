...
export function generateNewNode({ data, position, id, zIndex, type, ...rest }: Omit<Node, 'id'> & { id?: string }): {
  ...
  if (data.type === BlockEnum.Iteration) {
    ...
    (newNode.data)._children = [newIterationStartNode.id]
    return {
      newNode,
...
interface ParallelInfoItem {
  parallelNodeId: string
  depth: number
  isBranch?: boolean
}
interface NodeParallelInfo {
  parallelNodeId: string
  edgeHandleId: string
  depth: number
}
...
export const getParallelInfo = (nodes: Node[], edges: Edge[], parentNodeId?: string) => {
  ...
  if (parentNodeId) {
    ...
    startNode = nodes.find(node => node.id === (parentNode.data).start_node_id)
  }
  else {
